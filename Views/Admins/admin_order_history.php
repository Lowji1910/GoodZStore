...
$offset = ($page - 1) * $limit;
$sql_count = "SELECT COUNT(*) as total FROM orders WHERE status = 'completed' OR status = 'cancelled'";
$result_count = $conn->query($sql_count);
$total = $result_count ? intval($result_count->fetch_assoc()['total']) : 0;
$total_pages = ceil($total / $limit);
$sql = "SELECT o.*, u.full_name FROM orders o LEFT JOIN users u ON o.user_id = u.id WHERE o.status = 'completed' OR o.status = 'cancelled' ORDER BY o.created_at DESC LIMIT $limit OFFSET $offset";
