...
$sql = "SELECT p.*, c.name as category, i.image_url FROM products p LEFT JOIN categories c ON p.category_id = c.id LEFT JOIN product_images i ON p.id = i.product_id AND i.is_main = 1 ORDER BY p.created_at DESC LIMIT $limit OFFSET $offset";
